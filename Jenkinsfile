    ...
    stage('test') {
      parallel {
        stage('flake8') {
          steps {
            withDockerRegistry(url: "https://242567060652.dkr.ecr.us-west-2.amazonaws.com", credentialsId:"ecr:us-west-2:CIUser") {
              withDockerContainer(image: "242567060652.dkr.ecr.us-west-2.amazonaws.com/ros-ci/flake8:v2.1") {
        ...
        stage('cpplint') {
          steps {
            withDockerRegistry(url: "https://242567060652.dkr.ecr.us-west-2.amazonaws.com", credentialsId:"ecr:us-west-2:CIUser") {
              withDockerContainer(image: "242567060652.dkr.ecr.us-west-2.amazonaws.com/ros-ci/cpplint:v1.0") {
                sh label: 'Verify c++ code with cpplint', script: """
                  mkdir -p results
                  cpplint --output=junit --recursive ./ 2> results/cpplint_junit.xml || true
                """
              }
            }
          }
