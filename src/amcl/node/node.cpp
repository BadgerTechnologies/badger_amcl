...
void Node::savePoseToServer(const tf::Transform& latest_tf)
{
  if (!save_pose_)
  {
  ...
  tf::Pose map_pose = latest_tf.inverse() * latest_odom_pose_;
...
void Node::savePoseToFile()
{
  ...
  if (!latest_tf_valid_)
  {
    // we can enter this state on shutdown when the node makes a call directly to savePoseToFile
...
void Node::publishTransform(const ros::TimerEvent& event)
{
  ...
  if (tf_broadcast_ && getLatestTf(&tf_transform))
  {
    ...
    if (tf_reverse_)
    {
      tmp_tf_stamped = tf::StampedTransform(tf_transform, transform_expiration, odom_frame_id_, global_frame_id_);
    }
    else
    {
      tmp_tf_stamped = tf::StampedTransform(tf_transform.inverse(), transform_expiration,
...
bool Node::getLatestTf(tf::Transform* latest_tf)
{
  std::lock_guard<std::mutex> tfl(tf_mutex_);
  if(latest_tf_valid_)
  {
