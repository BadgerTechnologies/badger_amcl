...
void Node::init2D()
{
  ...
  private_nh_.param("global_localization_planar_non_free_space_factor", global_localization_non_free_space_factor_,
                    1.0);
  ...
  if (map_type_ == 2)
  {
    ...
    check_planar_scanner_timer_ =
        nh_.createTimer(planar_scanner_check_interval_, boost::bind(&Node::checkPlanarScanReceived, this, _1));
  }
}

void Node::checkPlanarScanReceived(const ros::TimerEvent& event)
{
 ...
 */
std::shared_ptr<OccupancyMap> Node::convertMap(const nav_msgs::OccupancyGrid& map_msg)
{
...
void Node::reconfigure2D(AMCLConfig& config)
{
  ...
  if (planar_model_type_ == PLANAR_MODEL_BEAM)
    planar_scanner_.setModelBeam(z_hit_, z_short_, z_max_, z_rand_, sigma_hit_, lambda_short_);
  else if (planar_model_type_ == PLANAR_MODEL_LIKELIHOOD_FIELD_PROB)
  {
    ROS_INFO("Initializing likelihood field model; this can take some time on large maps...");
    planar_scanner_.setModelLikelihoodFieldProb(z_hit_, z_rand_, sigma_hit_, sensor_likelihood_max_dist_, do_beamskip_,
  ...
  {
    ROS_INFO("Initializing likelihood field model; this can take some time on large maps...");
    planar_scanner_.setModelLikelihoodField(z_hit_, z_rand_, sigma_hit_, sensor_likelihood_max_dist_);
  ...
  else if (planar_model_type_ == PLANAR_MODEL_LIKELIHOOD_FIELD_GOMPERTZ)
  {
    ...
    planar_scanner_.setModelLikelihoodFieldGompertz(
        z_hit_, z_rand_, sigma_hit_, sensor_likelihood_max_dist_, planar_gompertz_a_, planar_gompertz_b_,
        planar_gompertz_c_, planar_gompertz_input_shift_, planar_gompertz_input_scale_, planar_gompertz_output_shift_);
    ROS_INFO("Gompertz key points by total planar scan match: "
             "0.0: %f, 0.25: %f, 0.5: %f, 0.75: %f, 1.0: %f",
             planar_scanner_.applyGompertz(z_rand_), planar_scanner_.applyGompertz(z_rand_ + z_hit_ * .25),
             planar_scanner_.applyGompertz(z_rand_ + z_hit_ * .5),
             planar_scanner_.applyGompertz(z_rand_ + z_hit_ * .75), planar_scanner_.applyGompertz(z_rand_ + z_hit_));
...
void Node::occupancyMapMsgReceived(const nav_msgs::OccupancyGridConstPtr& msg)
{
...
void Node::initFromNewOccupancyMap()
{
  planar_scanner_.init(max_beams_, occupancy_map_);
  if (planar_model_type_ == PLANAR_MODEL_BEAM)
    planar_scanner_.setModelBeam(z_hit_, z_short_, z_max_, z_rand_, sigma_hit_, lambda_short_);
...
void Node::planarScanReceived(const sensor_msgs::LaserScanConstPtr& planar_scan)
{
  ...
  if (frame_to_planar_scanner_.find(planar_scan->header.frame_id) == frame_to_planar_scanner_.end())
  {
    ...
    ROS_DEBUG("Received planar scanner's pose wrt robot: %.3f %.3f %.3f", planar_scanner_pose_v.v[0],
              planar_scanner_pose_v.v[1], planar_scanner_pose_v.v[2]);
  ...
  if (planar_scanners_update_[scanner_index])
  {
    ...
    planar_scanners_[scanner_index]->updateSensor(pf_, std::dynamic_pointer_cast<SensorData>(last_planar_data_));
...
void Node::globalLocalizationCallback2D()
{
  planar_scanner_.setMapFactors(global_localization_off_map_factor_, global_localization_non_free_space_factor_,
