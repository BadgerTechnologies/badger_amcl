...
// Create a new filter
ParticleFilter::ParticleFilter(int min_samples, int max_samples, double alpha_slow, double alpha_fast, double convergence_threshold,
...
void ParticleFilter::updateConverged()
{
  ...
  ros::NodeHandle nh("~");
  nh.getParam("global_localization_convergence_threshold", convergence_threshold_);
  std::shared_ptr<PFSampleSet> set;
  PFSample* sample;
  double total;
  ...
  if (percent_converged >= convergence_threshold_)
  {
    set->converged = true;
    converged_ = true;
